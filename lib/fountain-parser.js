...
function parse(script) {
  ...
  // --- Regex patterns ---
  const scene_heading_regex = /^(?:\s*)\.(.+)|(^(?:INT|EXT|EST|I\/E|INT\.?\/EXT\.?)\.?\s.*)/i;
  const transition_regex = /^>\s*(.+)|((?:[A-Z\s]+TO:)|CUT TO BLACK.|FADE (?:IN|OUT|TO BLACK)\.)$/;
  ...
  for (; i < lines.length; i++) {
    ...
    if (sceneMatch) {
      flushAction();
      scene_number++;
      // If group 1 exists, it's a forced heading. Otherwise, use group 2.
      const headingText = (sceneMatch[1] || sceneMatch[2] || '').trim();
    ...
    // Transitions
    const transMatch = trimmed.match(transition_regex);
    if (transMatch) {
      flushAction();
      // If group 1 exists, it's a forced transition. Otherwise, use group 2.
      const transText = (transMatch[1] || transMatch[2] || '').trim();
